...
].filter(cred => cred.username && cred.password); // 过滤无效凭据

// 认证函数：验证 Authorization 头
function authenticate(authHeader: string | null): boolean {
  if (!ADMIN_CREDENTIALS.length) {
    console.error("No valid admin credentials configured. Check ADMIN_USERNAME and ADMIN_PASSWORD environment variables.");
    return false;
  }
  ...
  console.log(`Authentication attempt: username=${inputUsername}, success=${isValid}`);
  return isValid;
}
